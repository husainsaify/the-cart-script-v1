<?php
	...
	// check the $_GET['id'] is set and its valid
	if (isset($_GET['id'])) {
		...
		<?php
			...
			if (isset($_POST['update'])) {
				...
				if (empty($name) || empty($desc) || empty($mp) || empty($sp) || empty($off) || $shipping == "" || empty($tags) || empty($category)) {
					...
					$update = $db->Update("product","category_id='$category',name='$name',description='$desc',mp='$mp',sp='$sp',off='$off',shipping='$shipping',tags='$tags'","id='$product_id'");
					if ($update) {
